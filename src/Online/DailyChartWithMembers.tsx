...
import {convertHourlyTSCodeToDateTime, getCurrentTimeSeconds, getMyTimeZone, isCacheOutdated} from "../utils";
import {CLAN_ICON_URL, DAILY_CHART_TTL_SECONDS} from "../utils/constants";
import {Activity} from "../types";

import './scss/styles-chart-with-members.scss';


export const DailyChartWithMembers = () => {

    const dispatch = useDispatch()
    const cache = useSelector((state: any) => state.dailyOnlineReducer, shallowEqual);
            ...
            if (activity.activity_map) {
                Object.keys(activity.activity_map).forEach((key) => {
                    if (activity.activity_map) {
                        result.push(
                            [
                                convertHourlyTSCodeToDateTime(key),
    ...
    const options = {
        ...
        xAxis: {
            ...
            labels: {
                formatter: (f: any) => {
                    return Highcharts.dateFormat('%H:00', f.value);
